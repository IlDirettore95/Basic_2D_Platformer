    ...
    public class PCGDataLoader
    {
        ...
        private void ExtractWFCTiles(PCGData data, XmlNode level, MonoBehaviour caller)
        {
            ...
            for (int i = 0; i < tilesList.Count; i++)
            {
                ...
                bool nIn = bool.Parse(node.Attributes["N_IN"].Value);
                bool nOut = bool.Parse(node.Attributes["N_OUT"].Value);
                bool eIn = bool.Parse(node.Attributes["E_IN"].Value);
                bool eOut = bool.Parse(node.Attributes["E_OUT"].Value);
                bool sIn = bool.Parse(node.Attributes["S_IN"].Value);
                bool sOut = bool.Parse(node.Attributes["S_OUT"].Value);
                bool wIn = bool.Parse(node.Attributes["W_IN"].Value);
                bool wOut = bool.Parse(node.Attributes["W_OUT"].Value);
                ...
                tile.PassabilityFlags[WFCTile.N_IN] = nIn;
                tile.PassabilityFlags[WFCTile.N_OUT] = nOut;
                tile.PassabilityFlags[WFCTile.E_IN] = eIn;
                tile.PassabilityFlags[WFCTile.E_OUT] = eOut;
                tile.PassabilityFlags[WFCTile.S_IN] = sIn;
                tile.PassabilityFlags[WFCTile.S_OUT] = sOut;
                tile.PassabilityFlags[WFCTile.W_IN] = wIn;
                tile.PassabilityFlags[WFCTile.W_OUT] = wOut;
